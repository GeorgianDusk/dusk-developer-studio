...
The clean `2157012` challenge returned **NO-GO** as well. Its material finding was integration drift: the still-enforced Phase 5 gate targeted 1.0.1 and synthetic flat receipts that the current expanded native assurance and OIDC publication workflows cannot emit. It also identified lower-severity artifact-ID consumption and bounded run-lookup precision gaps. CV-D-072 through CV-D-074 preserve those findings. Phase 5 now uses evidence schema 10 and the literal 1.0.2 workflow structures, validates main-push and cross-run bindings, and independently downloads the inner assurance payload plus reviewed package bytes; publication consumes the selected immutable artifact ID and queries GitHub with the exact candidate head SHA. Workflow-shaped positive and adversarial fixtures pass locally; re-freeze, native GitHub execution and another fresh challenge remain required.

Three additional source-blind public-1.0.1 discovery pilots exercised interruption, filesystem containment and redaction under serialized ownership of the fixed local ports. They remain non-counting and blocked where clean browser pairing or precise interruption timing was unavailable. Direct CLI containment safely rejected traversal, absolute/root, reparse, reserved-name, unsafe-character, boundary-length and overwrite attempts without changing sentinels or escape targets; Safe Mode exposed no fake secret or disposable-path canary in its UI, console, stdout, stderr or unprotected files. The interruption pilot found CV-D-075: a safe same-name direct CLI refusal incorrectly referred to a running Local Studio and gave no applicable recovery. The local repair keeps the refusal but gives mode-neutral new-name/empty-parent guidance, with unit and exact-package smoke coverage. All three disposable roots, processes and fixed-port listeners were independently cleaned; exact-candidate reruns remain required.

The first immutable `v1.0.2` publication attempt stopped safely before npm publication in GitHub run `29834025572`. Chrome 150 delivered and the Studio rendered the exact successful DuskDS preflight response, then Playwright observed a late `net::ERR_ABORTED` terminal event that the pairing-only classifier could not dispose. CV-D-076 records this P1 release-gate defect. The 1.0.3 repair classifies only the exact original page Request and Response after loopback endpoint, Origin, no-redirect, credentialed CORS, HTTP 200, authenticated-pairing order, authoritative contract, rendered UI and exclusive terminal-state checks pass; all unrelated failures remain fatal. Adversarial tests, full 346-test local verification and six consecutive real-Chrome exact-package runs pass. Clean native CI and immutable package proof remain pending.
...
| CV-D-072 | P1 | The retained Phase 5 gate targeted 1.0.1 and obsolete receipts that the current 1.0.2 workflows cannot emit. | Repaired locally with Phase 5 schema 10, literal expanded platform/nested assurance/cross-run publication contracts, main-push provenance and independent inner-payload/package artifact downloads; native CI retest pending. |
| CV-D-073 | P3 | Publication selected an immutable artifact ID but downloaded the main candidate by run-scoped name. | Repaired locally by emitting and consuming the exact artifact ID while retaining digest and byte checks. |
| CV-D-074 | P3 | Main-assurance lookup could miss a valid exact-commit run outside the newest 100 successes. | Repaired locally with GitHub's exact encoded `head_sha` filter plus the existing unique first-attempt predicate. |
| CV-D-075 | P3 | Direct `create-duskds` retry safely refused an existing target but misleadingly referred to a receipt from a running Local Studio. | Repaired locally with mode-neutral overwrite protection and actionable new-name/empty-parent recovery; exact-package smoke and final-candidate pilot retest pending. |
| CV-D-076 | P1 | The exact-package browser gate had no identity-bound disposition for a late Chrome abort on the already validated and rendered DuskDS preflight response, so the immutable v1.0.2 tag stopped before publication. | Repaired locally in 1.0.3 with exact Request/Response, endpoint, Origin, redirect, CORS, status, ordering, contract, UI and terminal-state checks; adversarial tests, full local verification and six Chrome package runs pass; native retest pending. |
