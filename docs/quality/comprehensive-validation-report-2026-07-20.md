...
| CV-D-064 | P1 | Final evidence could not be recorded without violating the requirement that evidence-ledger HEAD equal the already-packaged source commit. | Repaired locally by binding package bytes to the immutable tag while allowing only a clean descendant ledger commit whose changes are confined to explicit evidence/receipt/report paths; focused adversarial tests pass. |
