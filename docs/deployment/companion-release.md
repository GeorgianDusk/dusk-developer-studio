...
```bash
npx dusk-developer-studio@1.0.7
```
...
```bash
npx dusk-developer-studio@1.0.7 local-actions
```
...
```bash
npm exec --ignore-scripts --package=dusk-developer-studio@1.0.7 -- dusk-developer-studio
npm exec --ignore-scripts --package=dusk-developer-studio@1.0.7 -- dusk-developer-studio local-actions
```

Replace `1.0.7` with another reviewed version when needed. Never combine files from different package versions.
