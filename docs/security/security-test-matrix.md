...
| ST-13 Diagnostics | synthetic absolute paths/raw failures plus minimal health/scaffold responses | Responses contain allowlisted fields only; local paths and raw output are removed. |
| ST-14 Capability allowlist | strict request schema, portable project-name validation, and exact Dusk Forge command/argument assertion | Added command fields, shell metacharacter names, or modified tool shapes do not execute. |
