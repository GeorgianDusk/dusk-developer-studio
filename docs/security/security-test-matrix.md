...
assurance harness verifies the exact temporary SID, grants only its required
batch-logon right, removes any task-owned processes and profile, deletes the
task, removes the SID's complete LSA account-right record, and deletes the
account before it can pass; this task is CI scaffolding, not companion product
behavior. Its Windows limits are deliberately staggered: a five-minute child
lifecycle, a bounded 15-second post-kill confirmation, a six-minute scheduled
task limit, and a seven-minute parent observation limit. Linux and macOS use
`sudo` only to prove
