...
Unsigned RCs are internal-only. Standalone publication requires post-injection Authenticode on Windows, tag-bound keyless Sigstore on Linux, and Developer ID signing, hardened runtime, notarization, stapling, and Gatekeeper on macOS, with fresh-runner smoke for every target. Independent download/quarantine and reputation checks, security acceptance, support ownership, rollback, and explicit publication approval remain required. No signing key or generated private key belongs in the repository.
