...
import { isCompanionHealth, isCompanionSessionStatus, isPairingResult, isPreflightResult, isScaffoldEvidence } from "./responseSchemas";
import {
  DUSKDS_FORGE_COMMIT,
  DUSKDS_TEMPLATE_ID,
...
describe("companion response schemas", () => {
  it("accepts the bounded production shapes", () => {
    ...
    expect(isCompanionSessionStatus({ ok: true, paired: false })).toBe(true);
  ...
  it("rejects oversized, malformed, and capability-confused shapes", () => {
    ...
    expect(isCompanionSessionStatus({ ok: true, paired: false, capabilitiesEnabled: true })).toBe(false);
