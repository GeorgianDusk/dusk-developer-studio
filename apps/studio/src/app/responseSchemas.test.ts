...
describe("companion response schemas", () => {
  it("accepts the bounded production shapes", () => {
    ...
    expect(isCompanionHealth({ ok: true, service: "dusk-studio-local-agent", paired: true, capabilitiesEnabled: true, release: { product: "Dusk Developer Studio", version: "1.2.3", commit: "a".repeat(40), channel: "portable" } })).toBe(true);
