...
async function fixture(): Promise<string> {
  ...
  const manifest: PayloadManifest = {
    ...
    runtime: { name: "node", version: "24.18.0", archive_url: "https://nodejs.org/dist/v24.18.0/node.zip", archive_sha256: "b".repeat(64), binary_path: "runtime/node.exe", binary_sha256: sha("runtime") }, files
  };
...
describe("portable payload verification", () => {
  ...
  it("allows only the declared runtime to be externalized by a matching SEA host", async () => {
    ...
    const externalRuntime = { name: "node" as const, version: "24.18.0", binaryPath: "runtime/node.exe" };
    await expect(verifyPayload(root)).rejects.toThrow(/exact file set/);
